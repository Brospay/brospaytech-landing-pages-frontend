...
/**
 * Reconciliations Preview section component
 * Shows 6 benefits of reconciliations with link
 */
export function ReconciliationsPreview() {
  ...
  const benefits = [
    ...
    {
      title: 'Visibility',
      description: 'Accessibility and visibility of accurate complete data reduce potential fraudulent transactions and help to reveal hidden activity.',
      icon: (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    },
    {
      ...
      description: "BrosPay automates transaction matching across multiple PSPs and accounts, updating the data according to providers' statements.",
      icon: (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    },
    {
      ...
      description: 'Identify and manage mismatches, errors and collisions between transaction data from the provider and payment details inserted into BrosPay.',
      icon: (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {benefits.map((benefit, index) => (
            <div key={index} className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-3 font-heading">{benefit.title}</h3>
              <p className="text-gray-600 text-sm leading-relaxed font-body">{benefit.description}</p>
            </div>
          ))}
        </div>
